'''
Task 11 semanticscholar Details:
1. Given a DOI, return a list of Paper ojects citing this DOI with title, abstract, DOI, reference list in DOI (optional)
2. Used requests.post() to get batch data
3. Handle 429 with exponential backoff and jitter, max attempts 5. It has worked well in tests
4. Some papers cannot be found in semanticscholar (404), we just skip them
5. Sometimes paperId = None, we just skip
6. (to Yuntong) Still waiting for the cited_by function from Crossref:D
'''
import os
import sys
...
EMBEDDING_DIM = 1024 # should be 1024
BATCH_SIZE_PAPER_ID = 100
BATCH_SIZE_REF = 200
LIMIT = 100 # per page
MAX_ATTEMPTS = 5 # for 429
...
def get_all_citation_ids(doi: str) -> list[str]:
    ...
    while True:
        ...
        all_ids.extend([
            ...
            for c in data
            if "citingPaper" in c and c["citingPaper"].get("paperId")
        ])
        offset += LIMIT
...
def wait_time_429(resp, attempt, base=1, cap=30): # for 429
    ...
    if retry_after:
        try:
            ...
            pass
    # when there is no Retry-After，get a random wait time with exponential backoff and jitter
...
if __name__ == "__main__":
    ...
    # test case 2: a paper with 405 citations
    # cite = get_cite_semanticscholar("10.18653/v1/N18-3011")
    # with open("/home/ubuntu/openalex/workspace/zimo.li/task11_semanticscholar_crossref/batch_test.json", "w") as f:
    #     json.dump([paper.dict() for paper in cite], f, indent=2)

    # test case 3: the_doi, which is the second doi in dois
    get_cite_semanticscholar(the_doi)
    
    # test case 4: test batch_get_paper_by_paperId only
    # paperIds = [
